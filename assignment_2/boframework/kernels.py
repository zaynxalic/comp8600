...
class Matern(Kernel):
    ...
    def __call__(self, X: np.ndarray, Y: np.ndarray = None) -> np.ndarray:
        ...
        # if Y is not None, cdist(x,y) otherwise cdist(x,x)
        dist = cdist(X,X) if Y is None else cdist(X,Y) 
            
        if self.nu == 1.5:
            meatern = var * (1 + np.sqrt(3)* dist / length_scale) * np.exp(-np.sqrt(3)*dist/ length_scale)

        elif self.nu == 2.5:
            meatern = var * (1 + np.sqrt(5) * dist / length_scale + 5/3 * (dist/length_scale)**2 ) * np.exp(-np.sqrt(5)*dist/ length_scale)
        else:
            # Do not change
