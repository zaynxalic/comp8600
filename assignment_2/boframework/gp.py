...
class GPRegressor:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> GPRegressor:
        ...
        # define an objective function which locates the optimisation function
        def obj_func(theta):
            return -self.log_marginal_likelihood(theta)

        # First time optimization
        optima.append((self.optimisation(obj_func, self._kernel.theta, self._kernel.bounds)))

        # if n_restarts larger than 0
        if self.n_restarts > 0:
            for n_iter in range(self.n_restarts):
                theta_ = self.random_state.uniform(bounds[:, 0], bounds[:, 1])
                optima.append((self.optimisation(obj_func, theta_, self._kernel.bounds)))

        # Get the optimal results from history
        opt_theta = list(map(itemgetter(1), optima)) # find the minimum valur in tuple.
        self._kernel.theta = optima[np.argmin(opt_theta)][0]
    ...
    def predict(self, X: np.ndarray, return_std: bool = False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        x = self._X_train
        y = self._y_train
        n_targets = x.shape[1] # get #targets
        K_star  = kernel(x,X)
        K_star_t = kernel(X,x)# calculate K*T 
        Kx = kernel(x)
        Ky = Kx + self.noise_level * np.eye(Kx.shape[0]) # where self.noise_level = sigma**2

        L = cholesky(Ky, lower=True, check_finite=False)
        # solve the Ax = b equation given the Cholesky factorization of A using solve_triangular
        alpha = cho_solve((L, True), y, check_finite=False)
        y_mean = K_star_t@alpha

        if not return_std: # if not return std
            return y_mean
    
        else: # if return std
            v = solve_triangular(L, K_star, lower = True)
            K_star_star = kernel(X,X)
            y_var = K_star_star - v.T @ v
            y_std = np.sqrt(np.diag(y_var)) # calculate the standard deviation
            return y_mean.reshape(n_samples,n_targets), y_std.reshape(n_samples,n_targets)
    ...
    def fit_and_predict(self, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray,
        ...
        if optimise_fit:
            ...
            return kernel_params, self.predict(X_test,return_std)# if return_std = True, then return y_std as well.

        else:
            self._kernel = copy.deepcopy(self.kernel)
            self._X_train = X_train
            self._y_train = y_train
            return self.predict(X_test,return_std) # if return_std = True, then return y_std as well.
    ...
    def log_marginal_likelihood(self, theta: np.ndarray) -> float:
        ...
        kernel = self._kernel
        kernel.theta = theta
        x = self._X_train  # get x_train
        y = self._y_train  # get y_train
        n = x.shape[0]
        Kx = kernel(x)
        Ky = Kx + self.noise_level * np.eye(Kx.shape[0]) # where self.noise_level = sigma**2
        L = cholesky(Ky, lower=True, check_finite=False)
        ...
        log_likelihood = float(log_likelihood.squeeze())
